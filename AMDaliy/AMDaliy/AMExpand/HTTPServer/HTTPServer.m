...
@interface HTTPServer () <WebSocketDelegate>
...
- (id)init
{
	if ((self = [super init]))
	{
		...
		self.webSockets  = [[NSMutableArray alloc] init];
...
{
    for (WebSocket *ws in self.webSockets) {
        if (ws->isStarted) {
...
- (void)webSocketDidDie:(NSNotification *)notification
{
	...
	HTTPLogTrace();
	[self.webSockets removeObject:[notification object]];
